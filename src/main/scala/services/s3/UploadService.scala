...
class UploadServiceImpl[F[_] : Async : Logger](
  ...
  override def upload(key: String, contentType: String, data: Stream[F, Byte]): F[Unit] =
    for {
      _ <- Logger[F].debug(s"[UploadService] Preparing to upload file to bucket: $bucket, key: $key")
      bytes <- data.compile.to(Array)
      _ <- Logger[F].debug(s"[UploadService] Byte array compiled with size: ${bytes.length}")
      _ <- client.putObject(bucket, key, contentType, bytes)
      _ <- Logger[F].debug(s"[UploadService] Upload complete to key: $key")
    } yield ()

  override def generatePresignedUrl(key: String, fileName: String): F[Uri] =
    for {
      _ <- Logger[F].debug(s"[UploadService] Generating presigned GET URL for key: $key")
      uri <- presigner.presignGetUrl(bucket, key, fileName, Duration.ofMinutes(15))
      _ <- Logger[F].debug(s"[UploadService] Generated presigned GET URL: $uri")
    } yield uri

  override def generatePresignedUploadUrl(key: String): F[Uri] =
    for {
      _ <- Logger[F].debug(s"[UploadService] Generating presigned PUT URL for key: $key")
