...
trait DevBidRepositoryAlgebra[F[_]] {
...
class DevBidRepositoryImpl[F[_] : Concurrent : Monad : Logger](transactor: Transactor[F]) extends DevBidRepositoryAlgebra[F] {
  ...
  override def getDevBids(questId: String, limit: Int, offset: Int): F[List[GetDevBid]] = {
    ...
    for {
      _ <- Logger[F].debug(s"[DevBidRepositoryImpl][getDevBids] Fetching dev bids: questId=$questId, limit=$limit, offset=$offset")
      result <- query.transact(transactor)
      _ <- result.traverse_(bid => Logger[F].debug(s"[DevBidRepositoryImpl][getDevBids] Fetched bid: ${bid}"))
    } yield result
  }
...
object DevBidRepository {
  def apply[F[_] : Concurrent : Monad : Logger](transactor: Transactor[F]): DevBidRepositoryAlgebra[F] =
    new DevBidRepositoryImpl[F](transactor)
}
